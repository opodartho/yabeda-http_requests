  ...
  module HttpRequests
    ...
    class Sniffer
      def request(data_item)
        yield
        Yabeda.http_external_request_total.increment(
          {
      ...
      def response(data_item)
        yield
        log_http_external_response_total(data_item)
        log_http_external_response_duration(data_item)
      end

      private

      def log_http_external_response_total(data_item)
        Yabeda.http_external_response_total.increment(
          {
            host: data_item.request.host,
            port: data_item.request.port,
            method: data_item.request.method,
      ...
      def log_http_external_response_duration(data_item)
        labels = {
          host: data_item.request.host,
          port: data_item.request.port,
          method: data_item.request.method,
        ...
        Yabeda.http_external_response_duration.measure(
